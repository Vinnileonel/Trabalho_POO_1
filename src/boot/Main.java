...
public class Main {
    ...
    /** Converte um valor em euros para cêntimos. */
    private static long €(double valor) {
        return Math.round(valor * 100);
    }

    public static void main(String[] args) {
        // 1) Inicializar inventário e carregar dados:
        Inventario inventario = new Inventario();
        criarProdutos(inventario);
        criarCupoes(inventario);
        criarCartoes(inventario);

        // 2) Arrancar UI nas duas janelas:
        SwingUtilities.invokeLater(() -> {
            JanelaCompra jc = new JanelaCompra(
                "Loja HonESta de Castelo Branco", inventario);
            jc.setLocation(20, 20);
            jc.setVisible(true);

            JanelaCartao jcard =
                new JanelaCartao(inventario.todosCartoes());
    ...
    private static void criarProdutos(Inventario inv) {
        ...
        // Dois produtos extra definidos pelo grupo:
